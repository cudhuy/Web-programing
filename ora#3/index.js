...
const applyChange = (event) => {
	if (event.keyCode == 13) {
		const inputEl = event.target;
		const textEl = inputEl.previousElementSibling;
		textEl.textContent = inputEl.value + '_20194780';
		textEl.style.display = 'block';
		inputEl.style.display = 'none';
	}
};
...
const applyChangeNoMSSV = (event) => {
	if (event.keyCode == 13) {
		const inputEl = event.target;
		const textEl = inputEl.previousElementSibling;
		if (inputEl.value != '') textEl.textContent = inputEl.value;
		textEl.style.display = 'block';
		inputEl.style.display = 'none';
	}
};
...
const addGroupItem = (event) => {
    ...
    <div class="group_header">
        <div class="group_name" ondblclick="switchToInput(event)">Group Item_20194780</div>
